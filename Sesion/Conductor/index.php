<?php 
	...
	if(!$_POST == null){
		if(isset($_POST['cor']) && isset($_POST['pass'])){
			if($Usuario->ExisteConductores($_POST['cor'], $_POST['pass'])){
				$data = $Usuario->ExistConductor($_POST['cor']);
				$id = $Usuario->getIdUsuario($Alias, 2);
				$Consultas->actualizarEstado($id, "conductor");
				if($data != false){
